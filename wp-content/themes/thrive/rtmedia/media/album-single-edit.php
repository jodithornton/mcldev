...
<div class="rtmedia-container rtmedia-single-container rtmedia-media-edit">
	<?php if ( rtmedia_is_global_album( $rtmedia_query->media_query[ 'album_id' ] ) ) { ?>
		<h2><?php echo __( 'Edit Album : ', 'thrive' ) . esc_attr( $media[ 0 ]->media_title ); ?></h2>

		<div class="rtmedia-edit-media-tabs rtmedia-editor-main">
			<ul class="rtm-tabs clearfix">
				<li class="active"><a href="#details-tab"><i class='dashicons dashicons-edit rtmicon'></i><?php _e( 'Details', 'thrive' ); ?></a></li>
				<?php if ( ! is_rtmedia_group_album() ) { ?>
					<li class=""><a href="#manage-media-tab"><i class='dashicons dashicons-list-view rtmicon'></i><?php _e( 'Manage Media', 'thrive' ); ?></a></li>
				<?php } ?>
				...
				<div class="content active" id="details-tab">
					<form method="post" class="rtm-form">
						...
						<div class="rtmedia-edit-title rtm-field-wrap">
							<label for="media_title"><?php _e( 'Title : ', 'thrive' ); ?></label>
							<?php rtmedia_title_input(); ?>
						</div>

						<div class="rtmedia-editor-description rtm-field-wrap">
							<label for='description'><?php _e( 'Description: ', 'thrive' ) ?></label>
							<?php
						...
						<div>
							<input type="submit" name="submit" class='rtmedia-save-album' value="<?php _e( 'Save Changes', 'thrive' ); ?>" />
							<a class="button rtm-button rtm-button-back" href="<?php rtmedia_permalink(); ?>"><?php _e( 'Back', 'thrive' ); ?></a>
						</div>
					</form>
				</div>
					...
					<div class="content" id="manage-media-tab">
						<?php if ( have_rtmedia() ) { ?>
							<form class="rtmedia-album-edit rtmedia-bulk-actions" method="post" name="rtmedia_album_edit">
								...
								<p>
									<span><input type="checkbox" name="rtm-select-all" class="select-all" title="<?php _e( 'Select All Visible', 'thrive' ); ?>" /></span>
									<button class="button rtmedia-move" type='button' title='<?php echo __( 'Move Selected media to another album.', 'thrive' ); ?>' ><?php _e( 'Move', 'thrive' ); ?></button>
									<input type="hidden" name="move-selected" value="move">
									<button type="button" name="delete-selected" class="button rtmedia-delete-selected" title='<?php echo __( 'Delete Selected media from the album.', 'thrive' ); ?>'><?php _e( 'Delete', 'thrive' ); ?></button>
								</p>

								<p class="rtmedia-move-container">
									<?php $global_albums = rtmedia_get_site_option( 'rtmedia-global-albums' ); ?>
									<span><?php _e( 'Move selected media to the album : ', 'thrive' ); ?></span>
									<?php echo '<select name="album" class="rtmedia-user-album-list">' . rtmedia_user_album_list() . '</select>'; ?>
									<input type="button" class="rtmedia-move-selected" name="move-selected" value="<?php _e( 'Move Selected', 'thrive' ); ?>" />
								</p>
								...
								?>
								<a id="rtMedia-galary-prev" <?php echo $display; ?> href="<?php echo rtmedia_pagination_prev_link(); ?>"><?php _e( 'Prev', 'thrive' ); ?></a>
								...
								if ( rtmedia_offset() + rtmedia_per_page_media() < rtmedia_count() )
									...
									$display = 'style="display:none;"';
								?>
								<a id="rtMedia-galary-next" <?php echo $display; ?> href="<?php echo rtmedia_pagination_next_link(); ?>"><?php _e( 'Next', 'thrive' ); ?></a>
							</form>
						<?php } else { ?>
							<p><?php _e( 'The album is empty.', 'thrive' ); ?></p>
						<?php } ?>
					</div>
				<?php } ?>
	...
	<?php } else { ?>
		<p><?php echo __( "Sorry !! You can not edit this album.", 'thrive' ); ?></p>
	<?php } ?>
</div>