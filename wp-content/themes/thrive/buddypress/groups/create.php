	...
	<form action="<?php bp_group_creation_form_action(); ?>" method="post" id="create-group-form" class="standard-form" enctype="multipart/form-data">
		...
		<div class="item-body" id="group-create-body">
			...
			<?php if ( bp_is_group_creation_step( 'group-details' ) ) : ?>
				...
				<h2 class="bp-screen-reader-text"><?php _e( 'Group Details', 'thrive' ); ?></h2>
			...
			<?php if ( bp_is_group_creation_step( 'group-settings' ) ) : ?>
				...
				<h2 class="bp-screen-reader-text"><?php _e( 'Group Settings', 'thrive' ); ?></h2>
				...
				<fieldset class="group-create-privacy">
					...
					<div class="radio">
						<label><input type="radio" name="group-status" value="public"<?php if ( 'public' == bp_get_new_group_status() || !bp_get_new_group_status() ) { ?> checked="checked"<?php } ?> /> <strong><?php _e( 'This is a public group', 'thrive' ); ?></strong></label>
						<ul>
				...
				<?php if ( bp_is_active( 'forums' ) ) : ?>
					...
					<h4><?php _e( 'Group Forums', 'thrive' ); ?></h4>

					<?php if ( bp_forums_is_installed_correctly() ) : ?>

						<p><?php _e( 'Should this group have a forum?', 'thrive' ); ?></p>

						<div class="checkbox">
							<label><input type="checkbox" name="group-show-forum" id="group-show-forum" value="1"<?php checked( bp_get_new_group_enable_forum(), true, true ); ?> /> <?php _e( 'Enable discussion forum', 'thrive' ); ?></label>
						</div>
					<?php elseif ( is_super_admin() ) : ?>

						<p><?php printf( __( '<strong>Attention Site Admin:</strong> Group forums require the <a href="%s">correct setup and configuration</a> of a bbPress installation.', 'thrive' ), bp_core_do_network_admin() ? network_admin_url( 'settings.php?page=bb-forums-setup' ) :  admin_url( 'admin.php?page=bb-forums-setup' ) ); ?></p>
			...
			<?php if ( bp_is_group_creation_step( 'group-avatar' ) ) : ?>
				...
				<h2 class="bp-screen-reader-text"><?php _e( 'Group Avatar', 'thrive' ); ?></h2>
				...
				<?php if ( 'upload-image' == bp_get_avatar_admin_step() ) : ?>
					...
					<div class="main-column">
						<p><?php _e( "Upload an image to use as a profile photo for this group. The image will be shown on the main group page, and in search results.", 'thrive' ); ?></p>

						<p>
							<label for="file" class="bp-screen-reader-text"><?php _e( 'Select an image', 'thrive' ); ?></label>

							<input type="file" name="file" id="file" />
							<input type="submit" name="upload" id="upload" value="<?php esc_attr_e( 'Upload Image', 'thrive' ); ?>" />
							<input type="hidden" name="action" id="action" value="bp_avatar_upload" />
						</p>

						<p><?php _e( 'To skip the group profile photo upload process, hit the "Next Step" button.', 'thrive' ); ?></p>
					</div><!-- .main-column -->
				...
				<?php if ( 'crop-image' == bp_get_avatar_admin_step() ) : ?>
					...
					<h4><?php _e( 'Crop Group Profile Photo', 'thrive' ); ?></h4>

					<img src="<?php bp_avatar_to_crop(); ?>" id="avatar-to-crop" class="avatar" alt="<?php esc_attr_e( 'Profile photo to crop', 'thrive' ); ?>" />

					<div id="avatar-crop-pane">
						<img src="<?php bp_avatar_to_crop(); ?>" id="avatar-crop-preview" class="avatar" alt="<?php esc_attr_e( 'Profile photo preview', 'thrive' ); ?>" />
					</div>

					<input type="submit" name="avatar-crop-submit" id="avatar-crop-submit" value="<?php esc_attr_e( 'Crop Image', 'thrive' ); ?>" />
			...
			<?php if ( bp_is_group_creation_step( 'group-invites' ) ) : ?>
				...
				<h2 class="bp-screen-reader-text"><?php _e( 'Group Invites', 'thrive' ); ?></h2>
				...
				<?php if ( bp_is_active( 'friends' ) && bp_get_total_friend_count( bp_loggedin_user_id() ) ) : ?>
					...
					<div class="main-column">
						...
						<div id="message" class="info">
							<p><?php _e('Select people to invite from your friends list.', 'thrive' ); ?></p>
						</div>
						...
						<?php if ( bp_group_has_invites() ) : ?>
							...
							<?php while ( bp_group_invites() ) : bp_group_the_invite(); ?>
								...
								<li id="<?php bp_group_invite_item_id(); ?>">
									...
									<div class="action">
										<a class="remove" href="<?php bp_group_invite_user_remove_invite_url(); ?>" id="<?php bp_group_invite_item_id(); ?>"><?php _e( 'Remove Invite', 'thrive' ); ?></a>
									</div>
								</li>
				...
				<?php else : ?>
					...
					<div id="message" class="info">
						<p><?php _e( 'Once you have built up friend connections you will be able to invite others to your group.', 'thrive' ); ?></p>
					</div>
			...
			<?php if ( 'crop-image' != bp_get_avatar_admin_step() ) : ?>
				...
				<div class="submit" id="previous-next">
					...
					<?php if ( !bp_is_first_group_creation_step() ) : ?>
						...
						<input type="button" value="<?php esc_attr_e( 'Back to Previous Step', 'thrive' ); ?>" id="group-creation-previous" name="previous" onclick="location.href='<?php bp_group_creation_previous_link(); ?>'" />
					...
					<?php if ( !bp_is_last_group_creation_step() && !bp_is_first_group_creation_step() ) : ?>
						...
						<input type="submit" value="<?php esc_attr_e( 'Next Step', 'thrive' ); ?>" id="group-creation-next" name="save" />
					...
					<?php if ( bp_is_first_group_creation_step() ) : ?>
						...
						<input type="submit" value="<?php esc_attr_e( 'Create Group and Continue', 'thrive' ); ?>" id="group-creation-create" name="save" />
					...
					<?php if ( bp_is_last_group_creation_step() ) : ?>
						...
						<input type="submit" value="<?php esc_attr_e( 'Finish', 'thrive' ); ?>" id="group-creation-finish" name="save" />
